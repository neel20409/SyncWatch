...
export default function RoomPage() {
  ...
  // Init socket — runs only on client after roomId is available
  useEffect(() => {
    if (!roomId) return;
    ...
    // Dynamically import socket.io-client to ensure client-only execution
    import("socket.io-client").then(({ io }) => {
      ...
      sock.on("reconnect_attempt", (n) => {
        setSocketStatus(`Reconnecting... (${n})`);
      });
  ...
  // When player becomes ready, flush pending state
  useEffect(() => {
        ...
        <header className="flex items-center justify-between px-4 py-3 border-b border-[#1E1E1E] shrink-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5 bg-[#161616] border border-[#262626] rounded-lg px-2 py-1">
              ...
              <span className="text-xs text-gray-400 hidden sm:block max-w-[140px] truncate">{socketStatus}</span>
            </div>
            ...
            <button onClick={handleCopyLink} className="px-3 py-1.5 text-xs border border-[#262626] text-gray-400 rounded-lg hover:border-[#FF3B3B] hover:text-white transition-colors">
              Share
            </button>
          </div>
        </header>
        ...
        <div className="flex flex-1 overflow-hidden">
          <div className="flex-1 flex flex-col min-w-0 p-4 gap-3">
            <div className="flex gap-2">
              ...
              />
              <button onClick={handleVideoSubmit} className="px-4 py-2.5 bg-[#FF3B3B] text-white text-sm font-semibold rounded-lg hover:bg-red-500 transition-colors whitespace-nowrap">
                Load Video
              </button>
            </div>
            ...
            <div className="flex-1 bg-[#161616] rounded-xl overflow-hidden border border-[#262626] relative min-h-[300px]">
              {currentVideoId ? (
                ...
                <div className="w-full h-full flex flex-col items-center justify-center text-center p-8">
                  ...
                  <p className="text-gray-600 text-xs mt-2">Both users will see the video load automatically</p>
                </div>
              )}
            </div>
          </div>
